...
#nds$steps[is.na(nds$steps)]<- value

bStepsNA <- sapply(nds$steps, function(abc) {is.na(abc)})
str(bStepsNA)

#is.na(nds$steps)[bStepsNA]<-v2$ave.steps[bStepsNA]

# find the indices of those elements, because they're the ones we need to change
indices <-  which(bStepsNA)
str(indices)

## then change those values to the means calcluated in v2 above
foo<-sapply(indices, function(abc) { nds$steps[is.na(nds$steps[abc])] <- v2$ave.steps[abc] })
#foo
